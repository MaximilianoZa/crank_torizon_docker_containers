...
typedef struct _gr_wrap				gr_wrap_t;
...
typedef union _gre_data_union {
	...
	int64_t     i64;	
	uint64_t    u64;
	double		f64;			///< Placeholder only, not currently supported in all cases.
	gr_float_t  f32;
	int32_t     i32;
	uint32_t    u32;
	int16_t     i16;
...
//Should only be used for local data, since it can vary accross different machines
#define	GR_DATA_FORMAT_INTPTR  (sizeof(intptr_t) == 8) ? GR_DATA_FORMAT_8u1 : GR_DATA_FORMAT_4u1 
